...
**คะแนนอัปเดตเองเมื่อทำแบบประเมินเพิ่ม (แก้ 2026-07-27 รอบ 2)**
- ลิงก์ "ทำแบบประเมิน" **เปิดแท็บใหม่** — ไม่งั้นผู้ใช้หลุดจากรายงาน (และเลขตั๋ว) ที่เพิ่งได้มา
- โปรไฟล์ฝั่งผู้ใช้ย้ายจาก `sessionStorage` → **`localStorage`** เพราะ sessionStorage แยกตามแท็บ แท็บใหม่เขียนแล้วแท็บรายงานไม่มีทางเห็น · ผลพลอยได้: กลับมาวันหลังผลเครื่องมือยังอยู่
- หน้ารายงานฟัง event `storage` → **ทำแบบประเมินในอีกแท็บเสร็จ คะแนนขยับเองทันที** (มีปุ่ม "อัปเดตคะแนน" เป็นทางสำรอง)
- แยก `computePillars()` ออกมาเป็น pure function ใช้ร่วมกันทั้งตอนสร้างรายงานและตอนอัปเดตสด
- ⚠️ ยังเป็น client-only ทั้งหมด ไม่มีข้อมูลขึ้นเซิร์ฟเวอร์ก่อนกดยินยอม

**🐛 แก้บั๊กคะแนนการนอน (2026-07-27)**
- เครื่องมือการนอนมี 2 โหมด: **A = แนะนำเวลาเข้านอน** (คืนแค่รายการเวลา) · **B = ประเมินการนอนจริง**
- เดิมโหมด A ก็ได้คะแนน **48%** ทั้งที่ไม่ได้ประเมินอะไรเลย (เพราะโค้ดเช็กแค่ว่ามีผลลัพธ์ไหม)
- ตอนนี้: โหมด A → "ยังไม่ได้ประเมิน" (ไม่ให้คะแนน) · โหมด B → ให้คะแนนตามเกณฑ์

**การแสดงเสาคะแนนที่ยังไม่ได้ประเมิน (แก้ 2026-07-27)**
- คะแนนแต่ละเสามาจากเครื่องมือ: **คุณภาพไข่/อสุจิ · โภชนาการรวม · สมดุลฮอร์โมน** ← `เช็กสารอาหาร` · **การนอน** ← `คำนวณการนอน`
- เดิม "ยังไม่ได้ประเมิน" กับ "ได้ 0% จริง" แสดงเหมือนกันหมด (แถบว่าง) → ผู้ใช้แยกไม่ออกและไม่มีทางไปทำต่อ
- ตอนนี้: ยังไม่ประเมิน = **แถบเส้นประว่าง + ข้อความ "ยังไม่ได้ประเมิน" + ลิงก์ "ทำแบบประเมิน …→"** · ได้ 0% จริง = แถบทึบ + เลข 0%
- เพิ่มบรรทัดบอกความครบถ้วน: "คะแนนนี้คิดจาก **N ใน 4 ส่วน** ที่ประเมินแล้ว" + ลิงก์ "สร้างแผนใหม่อีกครั้ง" (รายงานเป็น snapshot ไม่อัปเดตเอง)
- ตัดข้อความซ้ำใน "จุดที่เสริมได้" ที่เคยชวนไปทำเครื่องมือออก

**คำที่เปลี่ยนตามเพศ (ฝ่ายชาย)**
| จุด | หญิง | ฝ่ายชาย |
|---|---|---|
| เสาคะแนนแรกในรายงาน | คุณภาพไข่ | **คุณภาพอสุจิ** |
| หมายเหตุใต้เสา | จากอาหารบำรุงไข่ที่คุณกิน | จากอาหารบำรุงอสุจิที่คุณกิน |
| จุดแข็งเรื่องโปรตีน | โปรตีนบำรุงไข่ | โปรตีนบำรุงอสุจิ |
| โปรตีนที่แนะนำเติม | Ferty | **Ferta** |
| ช่วงมีโอกาสสูง | รอบถัดไป | รอบถัดไป **(ของคู่คุณ)** |
| คำถาม PCOS ใน /plan | ถาม | **ไม่ถาม** (และล้างค่าถ้าเคยติ๊กไว้) |
| คำอธิบายหน้า ART | บำรุงไข่/ผนังมดลูก | บำรุงอสุจิ ให้เข้ากับจังหวะการรักษาของคู่ |
| เครื่องมือนับวันไข่ตก | ใช้ของตัวเอง | **เปิดให้ทุกคนใช้ — ระบุชัดว่า "กรอกแทนคู่ได้"** (ตัดสินใจ 2026-07-27: ไม่ซ่อนเครื่องมือจากผู้ชาย เพราะการวางแผนร่วมกันคือสิ่งที่แบรนด์ต้องการ) |
